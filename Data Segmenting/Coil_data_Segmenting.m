...
if str2num(handles.segment_number.String)>0
    ...
    switch cont
        case 'Yes'
            recent = dir(handles.save_path_name.String);
            recent([recent.isdir]) = [];
            t = datetime({recent.date}','InputFormat','dd-MMM-yyyy HH:mm:ss');
            [b,idx] = sortrows(t);
            recent = recent(idx);
        ...
        if contains(handles.listing(fs).name,{'LP', 'LA', 'LH', 'RP', 'RA', 'RH','stim'}) && (handles.listing(fs).bytes>0)
            switch handles.params.system_code
                case 1 % Lasker Spike2 Coil Files
                    ...
                    MultipolarStim = contains(handles.filename,'MultipolarStim');
                    MultipolarRef = contains(handles.filename,'MultipolarRef');
                    ...
                    if length(handles.delin)<6
                        ...
                        sd = find(handles.delin>stimPos(1),1);
                        rd = find(handles.delin>refPos(1),1);
                        handles.Segment(fs).stim = str2num(handles.filename(stimPos(1)+4:handles.delin(sd)-1));
                        handles.Segment(fs).refNum = str2num(handles.filename(refPos(1)+3:handles.delin(rd)-1));
                    ...
                    if sinu
                        ...
                        if length(handles.delin)<6
                            ...
                            extra = length(stimPos) - 1;
                            handles.Segment(fs).rate = str2num(handles.filename(ratePos+4:handles.delin(12+extra)-1));
                        end
                        ...
                        elseif any(strfind(handles.filename,'phase1Dur'))
                            extra = length(stimPos) - 1;
                            p1dPos = strfind(handles.filename,'phase1Dur');
                            handles.Segment(fs).p1d = str2num(handles.filename(p1dPos+9:handles.delin(9+extra)-1));
                            p2dPos = strfind(handles.filename,'phase2Dur');
                            handles.Segment(fs).p2d = str2num(handles.filename(p2dPos+9:handles.delin(11+extra)-1));
                            handles.Segment(fs).ipg = str2num(handles.filename(ipgPos+3:handles.delin(13+extra)-1));
                            p1aPos = strfind(handles.filename,'phase1amp');
                            handles.Segment(fs).p1amp = str2num(handles.filename(p1aPos+9:handles.delin(8+extra)-1));
                            p2aPos = strfind(handles.filename,'phase2amp');
                            handles.Segment(fs).p2amp = str2num(handles.filename(p2aPos+9:handles.delin(10+extra)-1));
%                             if MultipolarRef || MultipolarStim
%                                 p1dPos = strfind(handles.filename,'phase1Dur');
%                                 handles.Segment(fs).p1d = str2num(handles.filename(p1dPos+9:handles.delin(11)-1));
%                                 p2dPos = strfind(handles.filename,'phase2Dur');
%                                 handles.Segment(fs).p2d = str2num(handles.filename(p2dPos+9:handles.delin(13)-1));
%                                 handles.Segment(fs).ipg = str2num(handles.filename(ipgPos+3:handles.delin(15)-1));
%                                 p1aPos = strfind(handles.filename,'phase1amp');
%                                 handles.Segment(fs).p1amp = str2num(handles.filename(p1aPos+9:handles.delin(10)-1));
%                                 p2aPos = strfind(handles.filename,'phase2amp');
%                                 handles.Segment(fs).p2amp = str2num(handles.filename(p2aPos+9:handles.delin(12)-1));
%                             else
%                                 p1dPos = strfind(handles.filename,'phase1Dur');
...
%                             end
                        ...
                        if MultipolarStim   %^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
                            if length(stimPos) == 3
                                handles.Segment(fs).MultipolarStim = 1;
                                handles.Segment(fs).MultipolarRef = 0;
                                handles.Segment(fs).SecondStim = str2num(handles.filename(handles.delin(6)+5:handles.delin(7)-1));
                                handles.Segment(fs).ThirdStim = str2num(handles.filename(handles.delin(7)+5:handles.delin(8)-1));
                                handles.Segment(fs).ForthStim = 0;
                            elseif length(stimPos) == 4
                                handles.Segment(fs).MultipolarStim = 1;
                                handles.Segment(fs).MultipolarRef = 0;
                                handles.Segment(fs).SecondStim = str2num(handles.filename(handles.delin(6)+5:handles.delin(7)-1));
                                handles.Segment(fs).ThirdStim = str2num(handles.filename(handles.delin(7)+5:handles.delin(8)-1));
                                handles.Segment(fs).ForthStim = str2num(handles.filename(handles.delin(8)+5:handles.delin(9)-1));
                            else
                                handles.Segment(fs).MultipolarStim = 1;
                                handles.Segment(fs).MultipolarRef = 0;
                                handles.Segment(fs).SecondStim = str2num(handles.filename(handles.delin(6)+5:handles.delin(7)-1));
                                handles.Segment(fs).ThirdStim = 0;
                                handles.Segment(fs).ForthStim = 0;
                            end
                            handles.exp_def.String = {'-MultipolarStim-'};
                        end
